...
import { CreateAllowanceInput, UpdateAllowanceInput } from '@harmony/shared/schemas/allowance.schema';

const prisma = new PrismaClient();

export class AllowanceService {
    static async getAll(tenantId: string): Promise<Advantage[]> {
    ...
    static async getById(id: string, tenantId: string): Promise<Advantage | null> {
        return prisma.advantage.findFirst({
            where: { id, tenantId }
        });
    }

    static async create(tenantId: string, data: CreateAllowanceInput): Promise<Advantage> {
     ...
     */
    static async assignToGrade(gradeId: string, advantageId: string, tenantId: string, customAmount?: number) {
        ...
        if (!grade || !advantage) throw new Error("Grade ou Avantage introuvable");
     ...
     */
    static async removeFromGrade(gradeId: string, advantageId: string, tenantId: string) {
